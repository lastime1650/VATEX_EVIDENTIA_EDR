#include "IOCTL.hpp"

namespace EDR
{
	namespace IOCTL
	{
		BOOLEAN IOCTL::INITIALIZE(HANDLE ProcessId, HANDLE APC_THREADID, PVOID APC_HANDLER)
		{
			if (!IOCTL::ConnectIOCTL() || !hDevice)
				return false;


			struct IOCTL_INIT_s init_req_data;
			init_req_data.input.User_AGENT_ProcessId = ProcessId;
			init_req_data.input.User_APC_ThreadId = APC_THREADID;
			init_req_data.input.User_APC_Handler_UserAddress = APC_HANDLER;
			init_req_data.output.is_success = -1;

			if (!DataToKernel(
				IOCTL_INIT,
				&init_req_data,
				sizeof(init_req_data)
			))
			{
				DisconnectIOCTL();
				return false;
			}

			return true;
		}
		BOOLEAN IOCTL::ConnectIOCTL()
		{
			hDevice = CreateFileW(
				IOCTL_Device_SymbolicName,
				GENERIC_READ | GENERIC_WRITE,
				0,
				NULL,
				OPEN_EXISTING,
				0,
				NULL
			);
			if (hDevice == INVALID_HANDLE_VALUE)
			{
				hDevice = NULL;
				return false;
			}

			return true;
		}
		VOID IOCTL::DisconnectIOCTL()
		{
			if (!hDevice || hDevice == INVALID_HANDLE_VALUE)
				return;
			CloseHandle(hDevice);
			hDevice = NULL;
		}
		BOOLEAN IOCTL::DataToKernel(DWORD IOCTLCODE, PVOID Data, SIZE_T DataSize)
		{
			DWORD returnbytes = 0;
			return (BOOLEAN)DeviceIoControl(
				hDevice,
				IOCTLCODE,

				Data,
				DataSize,
				Data,
				DataSize,

				&returnbytes,

				NULL
			);
		}
	}
}